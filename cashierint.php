...
if (isset($_POST['pay_order'])) {
    ...
    $orderResult = $conn->query("
        SELECT Customer_id, Cashier_id, Total_amount 
        FROM Orders 
        WHERE Order_id = $order_id
    ");

    if ($orderResult && $order = $orderResult->fetch_assoc()) {
        ...
        if ($amount_paid >= $total_amount) {
            ...
            $message = "Payment successful! Change: ₱" . number_format($change,2);
...
// Fetch most recently added customer
